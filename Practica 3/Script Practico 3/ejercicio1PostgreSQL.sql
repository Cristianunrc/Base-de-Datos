...
DROP TABLE IF EXISTS ejercicio1.cliente;
...
-- 1_
-- a)
--select * from ejercicio1.producto where (precio > 550);
-- b)
--select cod_producto, descripcion, cantidad from ejercicio1.producto inner join ejercicio1.itemfactura using (cod_producto) where (cantidad > 5)
--order by (descripcion);
-- c)
--select nro_cliente, apellido, nombre, direccion, telefono from ejercicio1.cliente inner join ejercicio1.factura using (nro_cliente) inner join ejercicio1.itemfactura
--using (nro_factura) where (cantidad = 0) order by apellido desc;
-- d)
--select cod_producto, descripcion, producto.precio, cantidad from ejercicio1.producto inner join ejercicio1.itemfactura using (cod_producto) where (cantidad = 0);
--e)
--select cod_producto, descripcion, producto.precio, cantidad from ejercicio1.producto inner join ejercicio1.itemfactura using (cod_producto)
--where (cantidad > 3);
--f)
--select nro_factura, cliente.nro_cliente, apellido, nombre from ejercicio1.cliente left outer join ejercicio1.factura on (cliente.nro_cliente = factura.nro_cliente);

-- calcular el promedio de la cantidad de productos vendidos por cada producto
--select cod_producto, descripcion, avg (cantidad) as prom_cant from ejercicio1.producto inner join ejercicio1.itemfactura using (cod_producto) group by (cod_producto)
